...
#             cv2.putText(display_image, f'{idx}', tuple(centroid + np.array([0, 20])), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
#                         (0, 0, 255), 2)  # Display index
#         elif 'box' in det:
...
#             cv2.putText(display_image, f'{idx}', (x_min, y_max + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255),
#                         2)  # Display index
#
...
#
#
# def manual_labeling(image, json_path, initial_detections):
...
#                             cv2.circle(image_copy, tuple(point), max(1, int(5 / p)), (0, 0, 255), -1)
#                         points = select_polygon(image_copy, display_image_copy, p)
#                         if len(points) < 3:
#                             print("A polygon must have at least 3 points.")
#                             continue
...
#             annotations, labeled_image = manual_labeling(image, json_path, initial_detections)
#
#             # Ensure labeled image save folder exists
#             os.makedirs(labeled_image_save_folder, exist_ok=True)
#
#             # Save labeled image
#             labeled_image_path = os.path.join(labeled_image_save_folder, f"{stem}_manual_labeled.jpg")
